  ...
  <div class="">
    ...
    <div v-if="loading" class="space-y-4">
      <div v-for="i in 3" :key="i" class="animate-pulse flex items-center space-x-4 p-3">
        <div class="rounded-full bg-zinc-200 dark:bg-zinc-800 h-10 w-10"></div>
    ...
    <div v-else-if="error" class="p-4 bg-red-50 dark:bg-red-950/50 text-red-600 dark:text-red-400 rounded-lg text-sm">
      {{ error }}
    </div>
    ...
    <div v-else-if="commits.length > 0" class="relative pl-6 border-l-2 border-zinc-200 dark:border-zinc-800 space-y-8">
      <div v-for="item in commits" :key="item.sha" class="relative group bg-zinc-500/25 hover:bg-zinc-400/25 py-2 px-4 rounded transition shadow-md shadow-black/25">
        ...
        <div class="flex items-center gap-2 mt-2 text-xs text-zinc-500 dark:text-zinc-400">
          <img v-if="item.author?.avatar_url" :src="item.author.avatar_url" alt="Avatar" class="w-5 h-5 rounded-full"/>
          <span class="font-medium text-zinc-700 dark:text-zinc-300">{{ item.commit.author.name }}</span>
          <span>•</span>
    ...
    <div v-else class="text-center py-8 text-zinc-500 dark:text-zinc-400 text-sm">
      No commits found.
    </div>
  </div>
</template>