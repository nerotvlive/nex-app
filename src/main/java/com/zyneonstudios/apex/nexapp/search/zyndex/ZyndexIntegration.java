package com.zyneonstudios.apex.nexapp.search.zyndex;
...
public class ZyndexIntegration {
    ...
    public static boolean install(Instance instance, File installDir) {
        ...
        if(installInstance(instance,installDir)) {
            NEXApplication.getInstance().getInstanceManager().addInstance(new LocalInstance(installDir+"/zyneonInstance.json"));
            System.gc();
            return true;
        }
        System.gc();
    ...
    private static boolean installInstance(Instance instance, File installDir) {
        try {
            Path path = Paths.get(NEXApplication.getInstance().getWorkingDir() +"/temp/"+ UUID.randomUUID() +".zip");
            Download download = new Download(instance.getName(), URI.create(instance.getDownloadUrl()).toURL(),path);
            NEXApplication.getInstance().getDownloadManager().addDownload(download);
            ...
            NEXApplication.getLogger().err("[Minecraft] (ZyndexIntegration) Couldn't install instance "+instance.getId()+" v"+instance.getVersion()+": "+e.getMessage());
        }
        return false;
    }
