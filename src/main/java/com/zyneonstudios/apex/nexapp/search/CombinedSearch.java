package com.zyneonstudios.apex.nexapp.search;
...
import com.zyneonstudios.apex.nexapp.search.curseforge.search.CurseForgeSearch;
import com.zyneonstudios.apex.nexapp.search.curseforge.search.facets.CurseForgeFacetsBuilder;
import com.zyneonstudios.apex.nexapp.search.modrinth.search.ModrinthSearch;
import com.zyneonstudios.apex.nexapp.search.modrinth.search.facets.ModrinthFacetsBuilder;
import com.zyneonstudios.apex.nexapp.search.modrinth.search.facets.ModrinthProjectType;
import com.zyneonstudios.apex.nexapp.search.modrinth.search.facets.categories.ModrinthCategory;
import com.zyneonstudios.apex.nexapp.search.zyndex.ZyndexSearch;
import com.zyneonstudios.apex.nexapp.utilities.StringUtility;
...
public class CombinedSearch {
    ...
    public CombinedSearch(int[] curseForgeCategoryIds, ModrinthCategory[] modrinthCategories) {
        NEXSearch = new ZyndexSearch(NEXApplication.getInstance().getNEX());
    ...
    public JsonArray search(String query) {
        ...
        if (NEXApplication.getInstance().getLocalSettings().isDiscoverSearchNEX()) {
            nexResults = NEXSearch.search(query);
        } else {
            nexResults = new ArrayList<>();
        }

        if (NEXApplication.getInstance().getLocalSettings().isDiscoverSearchCurseForge()) {
            curseForgeResults = curseForgeSearch.search();
        } else {
            curseForgeResults = null;
        }

        if (NEXApplication.getInstance().getLocalSettings().isDiscoverSearchModrinth()) {
        ...
        try {
            ...
            NEXApplication.getLogger().printErr("NEX", "COMBINED SEARCH", "Couldn't process the search results...", e.getMessage(), e.getStackTrace());
        }
        return results;
    }
}