...
public class JavaUtilities {
    ...
    public static boolean installJava(String version, String path) {
        File tempFolder = new File(NEXApplication.getInstance().getWorkingPath()+"/temp/");
        ...
        try {
            ...
            NEXApplication.getInstance().getApplicationFrame().executeJavaScript("console.log('[CONNECTOR] settings.init.java');");
            System.gc();
            return true;
        } catch (Exception e) {
            NEXApplication.getLogger().printErr("NEX","JAVA INSTALLER","Couldn't install Java "+version+"!",e.getMessage(), e.getStackTrace());
        }
        System.gc();
        return false;
    }
    ...
    public static void setJava(MinecraftVersion.Type type) {
        try {
            NEXApplication.getLogger().log("Detected Minecraft version type " + type + "!");
            String javaHome = System.getProperty("java.home");
            if (type.equals(MinecraftVersion.Type.VERY_OLD)) {
                JavaUtil.setJavaCommand(null);
                String java = NEXApplication.getInstance().getLocalSettings().getJava8Path();
                if (!new File(java).exists()) {
                    NEXApplication.getLogger().log("Couldn't find compatible Java Runtime Environment! Starting download of Java 8...");
                    installJava("8", java);
                }
                javaHome = java;
            } else if (type.equals(MinecraftVersion.Type.OLD)) {
                JavaUtil.setJavaCommand(null);
                String java = NEXApplication.getInstance().getLocalSettings().getJava17Path();
                if (!new File(java).exists()) {
                    NEXApplication.getLogger().log("Couldn't find compatible Java Runtime Environment! Starting download of Java 17...");
                    installJava("17", java);
                }
                javaHome = java;
            } else if (type.equals(MinecraftVersion.Type.SEMI_NEW)) {
                JavaUtil.setJavaCommand(null);
                String java = NEXApplication.getInstance().getLocalSettings().getJava21Path();
                if (!new File(java).exists()) {
                    NEXApplication.getLogger().log("Couldn't find compatible Java Runtime Environment! Starting download of Java 21...");
                    installJava("21", java);
                }
                javaHome = java;
            } else if (type.equals(MinecraftVersion.Type.NEW)) {
                JavaUtil.setJavaCommand(null);
                String java = NEXApplication.getInstance().getLocalSettings().getJava25Path();
                if (!new File(java).exists()) {
                    NEXApplication.getLogger().log("Couldn't find compatible Java Runtime Environment! Starting download of Java 25...");
                    installJava("25", java);
                }
                javaHome = java;
            }
            ...
            NEXApplication.getLogger().log("Set Java Home to: " + javaHome);
        } catch (Exception e) {
            NEXApplication.getLogger().err("Couldn't set java version: " + e.getMessage());
        }
    }
