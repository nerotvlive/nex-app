package com.zyneonstudios.apex.nexapp.modules;
...
/**
 * The {@code ModuleLoader} class is responsible for loading, managing, and unloading modules within the NEX App.
...
public class ModuleLoader {
     ...
     */
    public ModuleLoader(NEXApplication application) {
        ...
        if (application.getSettings().has("settings.modules.uninstall")) {
            ...
            for (String module : uninstallModules) {
                if(!new File(module).delete()) {
                    NEXApplication.getLogger().deb("Failed to delete module " + module);
                }
            }
    ...
    public NexusModule readModule(File moduleJar) {
        try {
            ...
            try (JarFile jarFile = new JarFile(moduleJar.getAbsolutePath())) {
                ...
                JsonObject jsonObject = NEXApplication.getInstance().getFastGson().fromJson(reader, JsonObject.class);
                ...
                if (moduleJars.containsKey(id)) {
                    NEXApplication.getLogger().deb("Module with ID '" + id + "' is already loaded. Skipping duplicate.");
                    return null;
                }
            } catch (Exception e) {
                NEXApplication.getLogger().err("Couldn't read module " + moduleJar.getPath() + ": " + e.getMessage());
                return null;
            }
            ...
            if (id.equals(module.getModuleId())) {
                ...
                NEXApplication.getLogger().err("Module ID mismatch in " + moduleJar.getPath() + ". Expected: " + id + ", Found: " + module.getModuleId());
            }
        } catch (Exception e) {
            NEXApplication.getLogger().err("Couldn't read module " + moduleJar.getPath() + ": " + e.getMessage());
        }
    ...
    public void loadModule(NexusModule module) {
        ...
        if (!modules.containsKey(module.getModuleId())) {
            NEXApplication.getLogger().log("Loading module " + module.getModuleId() + " v" + module.getModuleVersion() + " by " + module.getModuleOwner() + "...");
            modules.put(module.getModuleId(), module);
            module.onLoad();
        } else {
            NEXApplication.getLogger().err("Module with ID '" + module.getModuleId() + "' is already loaded. Skipping.");
        }
    }
