...
import com.zyneonstudios.apex.nexapp.listeners.AsyncConnectorListener;
...
/**
 * The {@code AppFrame} class represents the main application window for the NEX App.
...
public class AppFrame extends NexusWebFrame implements ComponentListener, WebFrame {
    ...
    public AppFrame(NexusWebSetup setup, String url, boolean decorated) {
        super(setup.getWebClient(), url, decorated, decorated);
    ...
    private void initializeFrame(String url, NexusWebSetup setup, boolean decorated) {
        try {
            ...
            NEXApplication.getLogger().deb("Failed to load application icon: " + e.getMessage());
        }
    ...
    private void setupMenus(String url, NexusWebSetup setup, boolean decorated) {
        ...
        exit.addActionListener(e -> NEXApplication.stop(0));
        actions.add(exit);
        ...
        if (NEXApplication.getLogger().isDebugging()) {
        ...
        if (decorated && (!OperatingSystem.getType().equals(OperatingSystem.Type.Linux)|| NEXApplication.getLogger().isDebugging())) {
    ...
    private void setupDebugMenuItems(NexusWebSetup setup, boolean decorated) {
        ...
        disableDevtools.addActionListener(e -> {
            NEXApplication.getLogger().disableDebug();
            setVisible(false);
    ...
    private void handleWindowClosing() {
        ...
        if (!getTitle().contains("-clone ")) {
            if(!NEXApplication.getInstance().getLocalSettings().keepRunning()) {
    ...
    private void openInDefaultBrowser() {
        try {
            ...
            NEXApplication.getLogger().deb("Could not open URL in default browser: " + e.getMessage());
        }
    }
    ...
    public double getScaleFactor() {
        return NEXApplication.getInstance().getLocalSettings().getUiScale();
    }

    public void rescale() {
        getBrowser().setZoomLevel(getScaleFactor());
    }
}
