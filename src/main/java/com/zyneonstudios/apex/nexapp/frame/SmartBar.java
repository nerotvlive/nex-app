...
public class SmartBar extends JPanel {
    ...
    public SmartBar() {
        ...
        smartBarInput.addActionListener(e -> {
            if(NEXApplication.getInstance().getConsoleHandler().runCommand(smartBarInput.getText()+" ")) {
    ...
    private void validateValue() {
        CompletableFuture.runAsync(()->{
            try {
                Thread.sleep(10);
                if (!smartBarInput.getText().isEmpty()) {
                    if (NEXApplication.getInstance().getConsoleHandler().hasCommand(smartBarInput.getText().split(" ", 2)[0])) {
                        smartBarInput.setForeground(feedbackColor);
